...
abstract class RenderChartAxis extends RenderBox with ChartAreaUpdateMixin {
  ...
  set initialZoomFactor(double value) {
    if (_initialZoomFactor != value) {
      ...
      assert(initialZoomFactor >= 0 && initialZoomFactor <= 1, 'The initialZoomFactor of the axis should be between 0 and 1');
  ...
  set initialZoomPosition(double value) {
    if (_initialZoomPosition != value) {
      ...
      assert(initialZoomPosition >= 0 && initialZoomPosition <= 1, 'The initialZoomPosition of the axis should be between 0 and 1');
  ...
  void calculateVisibleRangeAndInvokeAnimation() {
    ...
    if (plotOffset > 0) {
      if (isVertical) {
        availableSize = Size(availableSize.width, availableSize.height - 2 * plotOffset);
      } else {
        availableSize = Size(availableSize.width - 2 * plotOffset, availableSize.height);
      }
    }
    ...
    if (!anchorRangeToVisiblePoints && isXAxis && parent!.behaviorArea?.zoomPanBehavior?.zoomMode == ZoomMode.x) {
      ...
      if (autoScrollingDelta != null && autoScrollingDelta! > 0 && !zoomingInProgress) {
        final DoubleRange autoScrollRange = updateAutoScrollingDelta(autoScrollingDelta!, newActualRange, newVisibleRange);
        if ((autoScrollingMode == AutoScrollingMode.end && newActualRange.minimum < autoScrollRange.minimum) || (autoScrollingMode == AutoScrollingMode.start && newActualRange.maximum > autoScrollRange.maximum)) {
  ...
  void _calculateRangeAndInterval(Size availableSize) {
    ...
    if (parent != null && parent!.onActualRangeChanged != null && (newActualRange != actualRange || newVisibleRange != visibleRange)) {
  ...
  ChartRangePadding effectiveRangePadding() {
    ...
    if (rangePadding == ChartRangePadding.auto) {
      if (isVertical) {
        padding = !isTransposed ? (_dependentIsStacked100 ? ChartRangePadding.round : ChartRangePadding.normal) : ChartRangePadding.none;
      } else {
        padding = isTransposed ? (_dependentIsStacked100 ? ChartRangePadding.round : ChartRangePadding.normal) : ChartRangePadding.none;
      }
    } else {
  ...
  void addNormalRange(DoubleRange range, num interval, Size availableSize) {
    ...
    if (range.minimum < 0) {
      ...
      minimum = range.minimum < ((5.0 / 6.0) * range.maximum) ? 0 : (range.minimum - (range.maximum - range.minimum) / 2);
      if (minimum % interval > 0) {
        minimum -= minimum % interval;
      }
    }
    num maximum = (range.maximum > 0) ? (range.maximum + (range.maximum - start) / 20) : (range.maximum - (range.maximum - start) / 20);
    remaining = interval - (maximum % interval);
    if ((0.365 * interval) >= remaining) {
      maximum += interval;
    }
    if (maximum % interval > 0) {
      maximum = range.maximum > 0 ? (maximum + interval) - (maximum % interval) : (maximum + interval) + (maximum % interval);
    }
  ...
  void _arrangeLabels(
    ...
    AxisLabel source;
    AxisLabel current = visibleLabels[0];
    final AxisLabel Function(AxisLabel, AxisLabel, double, _AlignLabel) applyLabelIntersectAction = _applyLabelIntersectAction();
    ...
    if (edgeLabelPlacement == EdgeLabelPlacement.hide) {
      ...
      source.position = betweenLabelsAlign(pointToPixel(source.value), source);
      current.position = betweenLabelsAlign(pointToPixel(current.value), current);
      if (effectiveLabelIntersectAction != AxisLabelIntersectAction.hide && effectiveLabelIntersectAction != AxisLabelIntersectAction.multipleRows) {
  ...
  AxisLabel Function(AxisLabel source, AxisLabel target, double extent, _AlignLabel align) _applyLabelIntersectAction() {
  ...
  bool _isHorizontalLabelIntersect(AxisLabel current, AxisLabel source) {
    if (current.position != null && source.position != null) {
      return !isInversed ? current.position! < source.position! + source.labelSize.width : current.position! + current.labelSize.width > source.position!;
    }
    return false;
  }
  ...
  bool _isVerticalLabelIntersect(AxisLabel current, AxisLabel source) {
    if (current.position != null && source.position != null) {
      return !isInversed ? current.position! + current.labelSize.height > source.position! : current.position! < source.position! + source.labelSize.height;
    }
    return false;
  }
  ...
  void generatePlotBands() {
    if (plotBands.isNotEmpty && associatedAxis != null && associatedAxis!.visibleRange != null) {
      ...
      final Rect Function(PlotBand plotBand, num start, num end) bounds = isVertical ? verticalPlotBandBounds : horizontalPlotBandBounds;
      for (int i = 0; i < length; i++) {
  ...
  num formPlotBandFrame(PlotBand plotBand, num current, num extent, num max, Rect Function(PlotBand plotBand, num start, num end) bounds) {
    ...
    current = plotBand.size != null ? plotBandExtent(plotBand, current, plotBand.isRepeatable ? plotBand.repeatEvery : end) : end;
    return current;
  }
  ...
  Rect horizontalPlotBandBounds(PlotBand plotBand, num start, num end) {
    ...
    if (plotBand.associatedAxisStart != null) {
      if (associatedAxis is RenderDateTimeCategoryAxis) {
        ...
        top = associatedAxis!.pointToPixel(associatedAxis!.actualValue(associateStart is num ? (associatedAxis! as RenderDateTimeCategoryAxis).indexToDateTime(associateStart as int) : associateStart));
      } else if (associatedAxis is RenderCategoryAxis) {
        final dynamic associateStart = plotBand.associatedAxisStart;
        top = associatedAxis!.pointToPixel(associatedAxis!.actualValue(associateStart is String ? (associatedAxis! as RenderCategoryAxis).labels.indexOf(associateStart) : associateStart));
      } else {
      ...
      if (associatedAxis is RenderLogarithmicAxis) {
        top = associatedAxis!.pointToPixel((associatedAxis! as RenderLogarithmicAxis).toPow(associatedAxis!.visibleRange!.minimum));
      } else {
    ...
    if (plotBand.associatedAxisEnd != null) {
      if (associatedAxis is RenderDateTimeCategoryAxis) {
        ...
        bottom = associatedAxis!.pointToPixel(associatedAxis!.actualValue(associateEnd is num ? (associatedAxis! as RenderDateTimeCategoryAxis).indexToDateTime(associateEnd as int) : associateEnd));
      } else if (associatedAxis is RenderCategoryAxis) {
        final dynamic associateEnd = plotBand.associatedAxisEnd;
        final num index = associateEnd is String ? (associatedAxis! as RenderCategoryAxis).labels.indexOf(associateEnd) : associateEnd;
        bottom = associatedAxis!.pointToPixel(associatedAxis!.actualValue(index != -1 ? index : associatedAxis!.actualRange!.maximum));
      } else {
        bottom = associatedAxis!.pointToPixel(associatedAxis!.actualValue(plotBand.associatedAxisEnd!));
      }
    } else {
      if (associatedAxis is RenderLogarithmicAxis) {
        bottom = associatedAxis!.pointToPixel((associatedAxis! as RenderLogarithmicAxis).toPow(associatedAxis!.visibleRange!.maximum));
      } else {
  ...
  Rect verticalPlotBandBounds(PlotBand plotBand, num start, num end) {
    ...
    if (plotBand.associatedAxisStart != null) {
      if (associatedAxis is RenderDateTimeCategoryAxis) {
        final dynamic associateStart = plotBand.associatedAxisStart;
        left = associatedAxis!.pointToPixel(associatedAxis!.actualValue(associateStart is num ? (associatedAxis! as RenderDateTimeCategoryAxis).indexToDateTime(associateStart as int) : associateStart));
      } else if (associatedAxis is RenderCategoryAxis) {
        final dynamic associateStart = plotBand.associatedAxisStart;
        left = associatedAxis!.pointToPixel(associatedAxis!.actualValue(associateStart is String ? (associatedAxis! as RenderCategoryAxis).labels.indexOf(associateStart) : associateStart));
      } else {
        left = associatedAxis!.pointToPixel(associatedAxis!.actualValue(plotBand.associatedAxisStart!));
      }
    } else {
      if (associatedAxis is RenderLogarithmicAxis) {
        left = associatedAxis!.pointToPixel((associatedAxis! as RenderLogarithmicAxis).toPow(associatedAxis!.visibleRange!.minimum));
      } else {
    ...
    if (plotBand.associatedAxisEnd != null) {
      if (associatedAxis is RenderDateTimeCategoryAxis) {
        final dynamic associateEnd = plotBand.associatedAxisEnd;
        right = associatedAxis!.pointToPixel(associatedAxis!.actualValue(associateEnd is num ? (associatedAxis! as RenderDateTimeCategoryAxis).indexToDateTime(associateEnd as int) : associateEnd));
      } else if (associatedAxis is RenderCategoryAxis) {
        final dynamic associateEnd = plotBand.associatedAxisEnd;
        final num index = associateEnd is String ? (associatedAxis! as RenderCategoryAxis).labels.indexOf(associateEnd) : associateEnd;
        right = associatedAxis!.pointToPixel(associatedAxis!.actualValue(index != -1 ? index : associatedAxis!.actualRange!.maximum));
      } else {
        right = associatedAxis!.pointToPixel(associatedAxis!.actualValue(plotBand.associatedAxisEnd!));
      }
    } else {
      if (associatedAxis is RenderLogarithmicAxis) {
        right = associatedAxis!.pointToPixel((associatedAxis! as RenderLogarithmicAxis).toPow(associatedAxis!.visibleRange!.maximum));
      } else {
  ...
  double pixelToPoint(Rect rect, double x, double y) {
    ...
    if (visibleRange != null) {
      return isVertical ? _coefficientToValue(1 - ((y - rect.top) / rect.height)) : _coefficientToValue((x - rect.left) / rect.width);
    }
    return double.nan;
  }
  ...
  void handleTapUp(TapUpDetails details) {
    if (parent != null && parent!.parentData != null && parent!.behaviorArea != null && (parent!.onAxisLabelTapped != null || hasTrimmedAxisLabel)) {
...
abstract class _AxisRenderer {
  ...
  Size _preferredSize(Size availableSize) {
    ...
    if (normalOrder) {
      ...
      multilevelLabelPadding = labelPadding + outerLabelSize + outerBorderGap + effectiveBorderSize + outerMultilevelLabelGap + effectiveMultilevelBorderSize;
      ...
      _axisBorderSize = effectiveBorderSize + effectiveMultilevelBorderSize + outerBorderGap + outerLabelSize + outerLabelGap + outerTickSize;
  ...
  void _drawTicks(PaintingContext context, Offset offset) {
    ...
    final Rect clipRect = axis.tickPosition == TickPosition.inside ? axis.parent!.plotAreaBounds : (axis.parentData! as CartesianAxesParentData).offset & axis.size;
  ...
  void _drawMultilevelLabels(PaintingContext context, Offset offset) {
    ...
    if (length > 0) {
      ...
      final Paint paint = Paint()
        ...
        ..strokeWidth = axis.multiLevelLabelStyle.borderWidth != 0 ? axis.multiLevelLabelStyle.borderWidth : axis.axisLine.width
        ..style = PaintingStyle.stroke;
      ...
      for (final AxisMultilevelLabel label in labels) {
        ...
        final TextSpan span = TextSpan(text: label.trimmedText, style: label.style.copyWith(color: label.style.color ?? axis.chartThemeData!.axisLabelColor));
        _textPainter
          ..text = span
          ..textAlign = TextAlign.center
          ..textDirection = TextDirection.ltr
          ..layout();
        final Rect bounds = Rect.fromLTWH(offset.dx + label.region.left, top + label.region.top, label.region.width, height);
    ...
    if (length > 0) {
      ...
      for (final AxisMultilevelLabel label in labels) {
        ...
        _textPainter
          ...
          ..textDirection = TextDirection.ltr
          ..layout();
        final Rect bounds = Rect.fromLTWH(left + label.region.left, offset.dy + label.region.top, width, label.region.height);
        _multilevelLabelBorderShape.onPaint(context, borderType, bounds, _textPainter, paint, axis);
      }
    }
  }
  ...
  void _drawTitle(PaintingContext context, Offset offset) {
    ...
    switch (axis.title.alignment) {
      ...
      case ChartAlignment.near:
        ...
        x = _labelOffset.dx + (_maxLabelSize - _textPainter.width);

        var a = axis.size.width * 2;
...
abstract class _MultilevelLabelBorderShape {
  ...
  void onPaint(
    ...
    final Rect clipRect = axis.labelPosition == ChartDataLabelPosition.inside ? axis.parent!.plotAreaBounds : (axis.parentData! as CartesianAxesParentData).offset & axis.size;
    final Rect extendedClipRect = clipRect.inflate(borderPaint.strokeWidth / 2);
    context.canvas.save();
    context.canvas.clipRect(extendedClipRect);
    final bool invertElementsOrder = axis.labelPosition == ChartDataLabelPosition.inside ? !axis.invertElementsOrder : axis.invertElementsOrder;
    switch (type) {
      case MultiLevelBorderType.rectangle:
  ...
  void _drawLabelAtRectangleBorderCenter(PaintingContext context, TextPainter labelPainter, Rect bounds) {
    labelPainter.paint(context.canvas, bounds.center.translate(-labelPainter.size.width / 2, -labelPainter.size.height / 2));
  }
  ...
  @protected
  void _drawSquareBraceBorder(PaintingContext context, Rect bounds, TextPainter labelPainter, Paint paint, bool isOpposed);

  @protected
  void _drawCurlyBraceBorder(PaintingContext context, Rect bounds, TextPainter labelPainter, Paint paint, bool isOpposed);
}
...
class _HorizontalMultilevelLabelBorderShape extends _MultilevelLabelBorderShape {
  ...
  void _drawNormalCurlyBraceBorder(Rect bounds, PaintingContext context, TextPainter labelPainter, Paint paint) {
    ...
    final Rect centerStartCurveRegion = Rect.fromLTWH(bounds.center.dx - _curveBraceCurveSize, startCurveRegion.bottom, _curveBraceCurveSize, _curveBraceCurveSize);

    path
      ..moveTo(startCurveRegion.left, startCurveRegion.top)
      ..quadraticBezierTo(startCurveRegion.bottomLeft.dx, startCurveRegion.bottomLeft.dy, startCurveRegion.bottomRight.dx, startCurveRegion.bottomRight.dy)
      ..lineTo(centerStartCurveRegion.left, centerStartCurveRegion.top)
      ..quadraticBezierTo(centerStartCurveRegion.topRight.dx, centerStartCurveRegion.topRight.dy, centerStartCurveRegion.bottomRight.dx, centerStartCurveRegion.bottomRight.dy);

    final Rect centerEndCurveRegion = Rect.fromLTWH(bounds.center.dx, centerStartCurveRegion.top, _curveBraceCurveSize, _curveBraceCurveSize);
    final Rect endCurveRegion = Rect.fromLTWH(right - _curveBraceCurveSize, startCurveRegion.top, _curveBraceCurveSize, _curveBraceCurveSize);

    path
      ..quadraticBezierTo(centerEndCurveRegion.topLeft.dx, centerEndCurveRegion.topLeft.dy, centerEndCurveRegion.topRight.dx, centerEndCurveRegion.topRight.dy)
      ..lineTo(endCurveRegion.left, endCurveRegion.bottom)
      ..quadraticBezierTo(endCurveRegion.bottomRight.dx, endCurveRegion.bottomRight.dy, endCurveRegion.topRight.dx, endCurveRegion.topRight.dy);
    context.canvas.drawPath(path, paint);
    // TODO(VijayakumarM): Add label padding.
    labelPainter.paint(context.canvas, Offset(bounds.center.dx - labelPainter.size.width / 2, bounds.top + 2 * _curveBraceCurveSize));
  }
  ...
  void _drawOpposedCurlyBraceBorder(Rect bounds, PaintingContext context, TextPainter labelPainter, Paint paint) {
    ...
    final Rect startCurveRegion = Rect.fromLTWH(left, bounds.bottom - _curveBraceCurveSize, _curveBraceCurveSize, _curveBraceCurveSize);
    final Rect centerStartCurveRegion = Rect.fromLTWH(bounds.center.dx - _curveBraceCurveSize, startCurveRegion.top - _curveBraceCurveSize, _curveBraceCurveSize, _curveBraceCurveSize);

    path
      ..moveTo(startCurveRegion.left, startCurveRegion.bottom)
      ..quadraticBezierTo(startCurveRegion.topLeft.dx, startCurveRegion.topLeft.dy, startCurveRegion.topRight.dx, startCurveRegion.topRight.dy)
      ..lineTo(centerStartCurveRegion.left, centerStartCurveRegion.bottom)
      ..quadraticBezierTo(centerStartCurveRegion.bottomRight.dx, centerStartCurveRegion.bottomRight.dy, centerStartCurveRegion.topRight.dx, centerStartCurveRegion.topRight.dy);

    final Rect centerEndCurveRegion = Rect.fromLTWH(bounds.center.dx, centerStartCurveRegion.top, _curveBraceCurveSize, _curveBraceCurveSize);
    final Rect endCurveRegion = Rect.fromLTWH(right - _curveBraceCurveSize, startCurveRegion.top, _curveBraceCurveSize, _curveBraceCurveSize);

    path
      ..quadraticBezierTo(centerEndCurveRegion.bottomLeft.dx, centerEndCurveRegion.bottomLeft.dy, centerEndCurveRegion.bottomRight.dx, centerEndCurveRegion.bottomRight.dy)
      ..lineTo(endCurveRegion.left, endCurveRegion.top)
      ..quadraticBezierTo(endCurveRegion.topRight.dx, endCurveRegion.topRight.dy, endCurveRegion.bottomRight.dx, endCurveRegion.bottomRight.dy);
    context.canvas.drawPath(path, paint);
    // TODO(VijayakumarM): Add label padding.
...
class _VerticalMultilevelLabelBorderShape extends _MultilevelLabelBorderShape {
  ...
  void _drawNormalCurlyBraceBorder(Rect bounds, PaintingContext context, TextPainter labelPainter, Paint paint) {
    ...
    final Rect centerStartCurveRegion = Rect.fromLTWH(startCurveRegion.left + _curveBraceCurveSize, bounds.center.dy - _curveBraceCurveSize, _curveBraceCurveSize, _curveBraceCurveSize);

    path
      ..moveTo(startCurveRegion.left, startCurveRegion.top)
      ..quadraticBezierTo(startCurveRegion.topRight.dx, startCurveRegion.topRight.dy, startCurveRegion.bottomRight.dx, startCurveRegion.bottomRight.dy)
      ..lineTo(centerStartCurveRegion.left, centerStartCurveRegion.top)
      ..quadraticBezierTo(centerStartCurveRegion.bottomLeft.dx, centerStartCurveRegion.bottomLeft.dy, centerStartCurveRegion.bottomRight.dx, centerStartCurveRegion.bottomRight.dy);

    final Rect centerEndCurveRegion = Rect.fromLTWH(centerStartCurveRegion.left, bounds.center.dy, _curveBraceCurveSize, _curveBraceCurveSize);
    final Rect endCurveRegion = Rect.fromLTWH(startCurveRegion.left, bottom - _curveBraceCurveSize, _curveBraceCurveSize, _curveBraceCurveSize);

    path
      ..quadraticBezierTo(centerEndCurveRegion.topLeft.dx, centerEndCurveRegion.topLeft.dy, centerEndCurveRegion.bottomLeft.dx, centerEndCurveRegion.bottomLeft.dy)
      ..lineTo(endCurveRegion.right, endCurveRegion.top)
      ..quadraticBezierTo(endCurveRegion.bottomRight.dx, endCurveRegion.bottomRight.dy, endCurveRegion.bottomLeft.dx, endCurveRegion.bottomLeft.dy);
    context.canvas.drawPath(path, paint);
    // TODO(VijayakumarM): Add label padding.
    labelPainter.paint(context.canvas, Offset(bounds.left + 2 * _curveBraceCurveSize, bounds.center.dy - labelPainter.size.height / 2));
  }
  ...
  void _drawOpposedCurlyBraceBorder(Rect bounds, PaintingContext context, TextPainter labelPainter, Paint paint) {
    ...
    final Rect startCurveRegion = Rect.fromLTWH(bounds.right - _curveBraceCurveSize, top, _curveBraceCurveSize, _curveBraceCurveSize);
    final Rect centerStartCurveRegion = Rect.fromLTWH(startCurveRegion.left - _curveBraceCurveSize, bounds.center.dy - _curveBraceCurveSize, _curveBraceCurveSize, _curveBraceCurveSize);

    path
      ..moveTo(startCurveRegion.right, startCurveRegion.top)
      ..quadraticBezierTo(startCurveRegion.topLeft.dx, startCurveRegion.topLeft.dy, startCurveRegion.bottomLeft.dx, startCurveRegion.bottomLeft.dy)
      ..lineTo(centerStartCurveRegion.right, centerStartCurveRegion.top)
      ..quadraticBezierTo(centerStartCurveRegion.bottomRight.dx, centerStartCurveRegion.bottomRight.dy, centerStartCurveRegion.bottomLeft.dx, centerStartCurveRegion.bottomLeft.dy);

    final Rect centerEndCurveRegion = Rect.fromLTWH(centerStartCurveRegion.left, bounds.center.dy, _curveBraceCurveSize, _curveBraceCurveSize);
    final Rect endCurveRegion = Rect.fromLTWH(startCurveRegion.left, bottom - _curveBraceCurveSize, _curveBraceCurveSize, _curveBraceCurveSize);

    path
      ..quadraticBezierTo(centerEndCurveRegion.topRight.dx, centerEndCurveRegion.topRight.dy, centerEndCurveRegion.bottomRight.dx, centerEndCurveRegion.bottomRight.dy)
      ..lineTo(endCurveRegion.left, endCurveRegion.top)
      ..quadraticBezierTo(endCurveRegion.bottomLeft.dx, endCurveRegion.bottomLeft.dy, endCurveRegion.bottomRight.dx, endCurveRegion.bottomRight.dy);
    context.canvas.drawPath(path, paint);
    // TODO(VijayakumarM): Add label padding.
...
abstract class ChartAxisController {
  ...
  double get zoomFactor => _zoomFactorTween != null && axis._animation != null ? _zoomFactorTween!.evaluate(axis._animation!) : _zoomFactor;
  ...
  double get zoomPosition => _zoomPositionTween != null && axis._animation != null ? _zoomPositionTween!.evaluate(axis._animation!) : _zoomPosition;
